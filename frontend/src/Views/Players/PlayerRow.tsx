    ...
    <Fragment>
      ...
      >
        ...
        <td className='px-1 w-[calc(40%-44px)] text-center'>
          ...
          <hr className={clsx('w-[90%] m-auto border-accent', player.IsSelected && 'border-primary-light')} />
      ...
      <tr
        ...
        className={clsx(player.IsSelected && 'bg-accent text-primary-light border-b-primary-light', 'border-2 border-t-0 border-accent')}
        onClick={() => selectPlayer(player.Id)}
        onKeyDown={() => selectPlayer(player.Id)}
      >
        <td colSpan={3}>
          <div className='flex flex-wrap flex-row w-[100%] gap-2 ml-2 mb-2'>
            {player.Conditions.map((tag, index) => (
                <div
                  className="rounded-full bg-primary-dark px-[.5em] py-1 text-primary-light"
                  key={index}
                >
