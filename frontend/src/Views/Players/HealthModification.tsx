  ...
  return (
    <Fragment>
      <div className='flex flex-row  place-content-between pt-6 lg:place-content-start lg:gap-y-8 lg:pt-0'>
        <PopoverButton
          buttonText="Damage"
          overrideButtonClassname="h-full w-full bg-calltoaction-red text-white"
          overridePopoverClassname="h-[60px] w-[128px] lg:ml-4 lg:w-32"
          overridePanelClassname="flex flex-col gap-y-2 rounded-lg border-2 bg-accent text-primary-light p-4 shadow-2xl"
          placement="top-end"
        >
          {negativeModificationValues.map((value: number) => {
            const stringValue = value > 0 ? `+${value}` : value;
            return <button key={value} className='bg-calltoaction-red text-white' onClick={() => applyModification(value)}>{stringValue}</button>;
          })}
        </PopoverButton>
        {/* TODO: Make widths less hardcoded */}
        <PopoverButton
          buttonText="Heal"
          overrideButtonClassname="h-full w-full bg-calltoaction text-black lg:ml-4 lg:w-32"
          overridePopoverClassname="h-[60px] w-[128px]"
          overridePanelClassname="flex flex-col gap-y-2 rounded-lg border-2 bg-accent text-primary-light p-4 shadow-2xl w-[150%]"
          placement="top-start"
        >
          <div className="grid grid-cols-2 gap-4 pr-2 border-r-black">
            ...
            {tempModificationValues.map((value: number) => {
              const stringValue = value > 0 ? `+${value}` : value;
              return <button key={value} className='bg-calltoaction text-black' onClick={() => applyTempModification(value)}>{stringValue}</button>;
            })}
          </div>
          <div className="col-span-2 grid grid-cols-2 gap-4">
            <h5 className="w-full border-b-2 border-black text-lg col-span-2">Normal HP</h5>
            <button className='bg-calltoaction text-black' onClick={() => applyModification(0)}>Max</button>
            {positiveModificationValues.map((value: number) => {
              const stringValue = value > 0 ? `+${value}` : value;
              return <button key={value} className='bg-calltoaction text-black' onClick={() => applyModification(value)}>{stringValue}</button>;
            })}
          </div>
        </PopoverButton>
      </div>
