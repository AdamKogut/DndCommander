    ...
    <div className={`relative z-0 m-2 mt-6 ${parentClassnameOverride ?? ''}`}>
      <textarea
        ...
        className={`peer block w-full appearance-none overflow-y-hidden rounded-t-lg border-0 border-b-2 border-gray-300 px-2.5 pb-2.5 pt-5 text-sm text-gray-900` +
          ...
          `dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:focus:border-blue-500 ${inputClassnameOverride ?? ''}`}
        placeholder=" "
        value={value}
        wrap='hard'
        rows={1}
      ...
      <label
        ...
        className={`absolute top-4 left-2.5 z-10 origin-[0] -translate-y-4 scale-75 text-sm text-gray-500 duration-300` +
          ...
          `dark:text-gray-400 peer-focus:dark:text-blue-500 ${labelClassnameOverride ?? ''}`}
      >
        {placeholder}
      </label>
    </div>
