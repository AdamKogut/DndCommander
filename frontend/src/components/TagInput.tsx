    ...
    <div className={`relative z-0 m-2 ${parentClassnameOverride}`}>
      <div className='flex flex-wrap gap-2 align-middle'>
        {!disabled && <input
          ...
          className={`peer block w-full appearance-none rounded-t-lg border-0 border-b-2 border-gray-300 px-2.5 pb-2.5 pt-5 text-sm text-gray-900 focus:border-blue-600 focus:outline-none focus:ring-0 ${inputClassnameOverride}`}
          placeholder=" "
          value={currentText}
        />}
        <label
          htmlFor={`floating_${idPlaceholder}`}
          className={`absolute top-4 left-2.5 z-10 origin-[0] -translate-y-4 scale-75 text-sm text-gray-500 duration-300 peer-placeholder-shown:translate-y-0 peer-placeholder-shown:scale-100 peer-focus:-translate-y-4 peer-focus:scale-75 peer-focus:text-blue-600 ${labelClassnameOverride}`}
        >
          {placeholder}
        </label>
        <div 
