  ...
  return (
    <Dialog open={isOpen} onClose={onInternalCancel} className='relative z-50'>
      ...
      <div className="fixed inset-0 flex h-full w-full items-center justify-center sm:h-auto">
        <Dialog.Panel className="primary-coloring mx-auto h-full w-full min-w-[33%] bg-primary-light sm:h-[80%] sm:max-h-[80%] sm:min-w-[33%] sm:max-w-[80%] sm:rounded lg:max-w-[55%] 2xl:max-w-[40%]">
          <Dialog.Title className='rounded-t-md bg-primary-dark p-2 text-primary-light text-xl'>{title}</Dialog.Title>
          <div className={clsx('h-[calc(100%-60px)] overflow-hidden', (onConfirm || onCancel || onClose) && 'h-[calc(100%-90px)]')}>
            {children}
          </div>

          <div className='flex justify-end gap-x-3 rounded-b-md bg-primary-dark p-2'>
