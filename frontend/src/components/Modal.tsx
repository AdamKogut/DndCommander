  ...
  return (
    <Dialog open={isOpen} onClose={onInternalCancel} className='relative z-50'>
      ...
      <div className="fixed inset-0 flex h-full w-full items-center justify-center sm:h-auto sm:p-4">
        <Dialog.Panel className="mx-auto h-full w-full min-w-[33%] bg-white sm:h-[80%] sm:max-h-[80%] sm:min-w-[33%] sm:max-w-[80%] sm:rounded sm:p-4 lg:max-w-[55%] 2xl:max-w-[40%]">
          ...
          <div className='flex justify-end gap-x-3 rounded-b-md bg-slate-200 p-2'>
            ...
            {onClose && <button className='btn-primary py-1' onClick={onInternalClose}>Close</button>}
          </div>
        </Dialog.Panel>
      </div>
    </Dialog>
